...
def _execute_job_uploading_lfs(
    ...
    try:
        ...
        logger.error(
            f"failed to preupload LFS {paths.file_path} slice {metadata.lfs_upload_part_index}/{metadata.lfs_upload_part_count}: {e}")
        traceback.format_exc()
...
def _get_upload_mode(
    ...
    if not files_modes and len(files_modes) != len(items):
        raise ValueError(
            f"requested {len(items)} files do not match {len(files_modes)} files in fetch upload modes response")
    ...
    for item in items:
        ...
        metadata.remote_oid = None if remote_file_oids[paths.path_in_repo] == "" else remote_file_oids[
            paths.path_in_repo]
        metadata.save(paths)
...
def _preupload_lfs_done(
    ...
    logger.debug(
        f"LFS file {paths.file_path} merge {metadata.lfs_upload_part_count} uploaded slices complete response: {complete_resp}")
...
def _preupload_lfs(
    ...
    if not isinstance(objects, list) or len(objects) < 1:
        raise ValueError(
            f"LFS {paths.file_path} malformed batch response objects is not list from server: {batch_resp}")
    object = objects[0]

    search_key = "actions"
    if not isinstance(object, dict) or search_key not in object:
        raise ValueError(
            f"no slices batch {search_key} info found for response of file {paths.file_path} from server: {object}")
    ...
    if not isinstance(object_upload, dict) or search_key not in object_upload:
        raise ValueError(
            f"no slices batch {search_key} found for response of file {paths.file_path} from server: {object}")
    object_upload_header = object_upload[search_key]

    href_key = "href"
    if not isinstance(object_upload, dict) or search_key not in object_upload:
        raise ValueError(
            f"no slices batch merge address found for response of file {paths.file_path} from server: {object}")
